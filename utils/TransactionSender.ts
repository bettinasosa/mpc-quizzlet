...
export class TransactionSender {
  ...
  private constructor(
    ...
    transactionValidityDuration: BN
  ) {
    this.client = client
  ...
  public sign(
    ...
    return Promise.all([
      this.client.getAccountState(sender),
      this.client.getChainId()
    ]).then(([account, chainIdData]) => {
  ...
  public send(signedTransaction: SignedTransaction): Promise<SentTransaction> {
    ...
    return this.client
      .putTransaction(fromAddress, signedTransaction)
  ...
  public sendAndSign(
    ...
    return this.sign(transaction, gasCost).then(signed => {
      if (signed === undefined) {
  ...
  public getAddress(): BlockchainAddress {
    return BlockchainAddress.fromString(
      CryptoUtils.keyPairToAccountAddress(this.keyPair)
    )
  }
}
